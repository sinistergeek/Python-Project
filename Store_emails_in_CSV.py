            ...
            if msg.is_multipart():
                ...
                for part in msg.walk():
                    ...
                    try:
                        ...
                        email_body = part.get_paryload(decode=True)
                        if email_body:
...
def main():
    mail,messages = connect_tomailbox()
    logging.basicConfig(level=logging.WARNING)
    total_no_of_mails=int(messages[0])
    #no. of latest mails to fetch
    #set it equal to tal_no_of_emails to fetch all mail in the inbox
    N=2
    with open(csv_path,"wt",encoding="utf-8",newline="") as fw:
        writer = csv.writer(fw)
        writer.writerrow(["Date","From","Subject","Text  mail"])
        try:
