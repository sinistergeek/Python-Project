...
def play():
    #Asking if the player is read.
    return input("\n Are you read to play the game? Enter [Y]es or [N]o.\t").upper().startswith('Y')
...
def choice():
    ...
    while p1_choice!='X' or p1_choice!='O': #while loop; if the entered value isn't X or O;
        ...
        #WhILE loop ends
    #Assigin the value to p2 and then displaying the values;
    if p1_choice=='X':
        p2_choice='O'
    elif p1_choice=='O':
        p2_choice ='X'
...
def CompAI(board,name,choice):
    ...
    #including both X and O, since if computer will win, he willplace a choice there, but if the component will win --> we have to block that move
    for let in ['O','X']:
        for i in possibilities:
            #creating a copy of the board everytime, placing the move and checking if it wins;
            #Creating copy like this and not this boardCopy=board,since changes to boardCopy changes the original board;
            boardCopy=board[:]
            boardCopy[i]=let
...
while True:
    #Creating the board as a list; to be ketp replacing it with user input:
    theBoard=[' ']*10
    ...
    if mode == 1:
        ...
        #Asking Choices; Printing choice; X or O;
    ...
    if(mode==2):
        ent = input("\n This is going to be fast! Press Enter the battle to begin!\n")
        play_game=1
    else:
        play_game=play()
    ...
    while play_game:
        ...
        if turn == p1_name:
            ...
            if win_check(theBoard,p1_choice):
                ...
                #To check if the board is full. if yes the game is draw;
            ...
            if(mode==1):
                ...
                print(f'\n{p2_name}({p2_choice}) has place on {position}\n')
            place_marker(theBoard,available,p2_choice,position)

                #Tp check if Player 2 has won after the current input;
